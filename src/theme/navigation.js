...
class Navigation extends Component {
  ...
  profile = () => {
    return (
      <div className="dropdown profile-element">
        <img alt="" className="rounded-circle" src={profileSmall}/>
  ...
  menu = () => {
    return this.state.menu.map((item, index) => {
      ...
      return (
        <MenuTree key={index} icon="building-o" label="General">
          {
            item.tree.map((treeItem, treeIndex) => {
              ...
              return (
                <MenuTree key={treeIndex} icon={'building-o'} label={'GEneral aa'} >
